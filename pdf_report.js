...
module.exports = function buildPdfHtml(ins, uploadsDir, branding) {
  ...
  var fileToDataUri = function(filePath) {
    if (!filePath || !fse.existsSync(filePath)) return '';
  ...
  var logoHtml = b.pdfLogoDataUri ? '<img class="cover-logo" src="' + b.pdfLogoDataUri + '" alt="Logo">' : '';
  var coverImagePath = r.coverImagePath ? path.join(uploadsDir || '', r.coverImagePath) : '';
  var coverImageHtml = coverImagePath ? fileToDataUri(coverImagePath) : '';
  var appName = E(b.appName || 'Auditor Libre');
  var cover = '<div class="cover-head">';
  if (logoHtml) cover += logoHtml;
  cover += '<div class="cover-head-text">';
  if (!logoHtml) cover += '<div class="app-name">' + appName + '</div>';
  cover += codeText;
  ...
  cover += '</div></div>';
  if (coverImageHtml) cover += '<div style="margin-top:14px"><img src="' + coverImageHtml + '" alt="Imagen de portada" style="width:100%;max-height:110mm;object-fit:cover;border-radius:14px;border:1px solid #e9e8e6;display:block"></div>';
  cover += '<hr class="div">';
  ...
  var buildQuestionContent = function(q, a) {
    ...
    if (ans.note) h += '<tr'+rowCls+'><td class="ql" style="font-weight:400;font-style:italic;color:#6b6b68;font-size:9pt">Nota:</td><td class="al">'+E(ans.note)+'</td></tr>';
    var actionDesc = ans.action ? (typeof ans.action === 'object' ? (ans.action.description || '') : ans.action) : null;
    if (actionDesc) h += '<tr'+rowCls+'><td class="ql">&#8594; Accion:</td><td class="al">'+E(actionDesc)+'</td></tr>';
    return h;
  };

  var buildMediaContent = function(q, a) {
    var mf = a ? a.mediaFiles : null;
    if (!mf || !mf.length) return '';
    var h = '<div class="mb">';
    mf.forEach(function(f,mi){
      h += '<div><img class="mi" src="'+imgSrc(f)+'"><div class="mc">Foto '+(mi+1)+'</div></div>';
    });
    return h + '</div>';
  };
  ...
  var buildPageContent = function(p) {
    ...
    p.sections.forEach(function(s) {
      if (s.repeatable) {
        var instances = ins.repeatableAnswers[s.id] || [];
        if (!instances.length) {
          h += '<div class="sh">'+E(s.title||'Seccion')+'</div>';
          h += '<table class="qt">';
          s.questions.forEach(function(q) { h += buildQuestionContent(q, null); });
          h += '</table>';
        } else {
  ...
  var buildFlagged = function() {
    ...
    var h = '<div class="pb"></div><div class="pdf-page"><div class="sh">ELEMENTOS SENALADOS Y ACCIONES</div><table class="qt">';
    items.forEach(function(it){
      ...
      var itActionDesc = it.a.action ? (typeof it.a.action === 'object' ? (it.a.action.description || '') : it.a.action) : null;
      if(itActionDesc) h += '<tr class="fl"><td class="ql">&#8594; Accion:</td><td class="al">'+E(itActionDesc)+'</td></tr>';
    });
    return h + '</table></div>';
  };
  var buildScore = function() {
    ...
    return '<div class="pb"></div><div class="pdf-page"><div class="sh">PUNTUACION</div><div style="padding:16px 12px"><div class="sp">'+pct2+'%</div><div class="ss">'+ins.score+' de '+ins.maxScore+' puntos</div><div class="sb-bg"><div class="sb-fi" style="width:'+pct2+'%"></div></div></div></div>';
  };
  var buildSig = function() {
    if(!ins.signature) return '';
    return '<div class="sig"><span class="sig-l">Firma responsable</span><img class="sig-i" src="'+ins.signature+'"><div class="sig-n">'+E(ins.signerName||'Inspector')+'</div>'+(ins.signerTitle?'<div class="sig-t">'+E(ins.signerTitle)+'</div>':'')+'<span class="sig-t">'+fmtD(ins.completedAt)+'</span></div>';
  };
  var buildMedia = function() {
    ...
    var h = '<div class="pb"></div><div class="pdf-page media-page"><div class="sh">RESUMEN DE ARCHIVOS MULTIMEDIA</div><div class="mg">';
    all.forEach(function(m,i){ h += '<div class="mgi"><img class="mgg" src="'+imgSrc(m.file)+'"><div class="mc">Foto '+(i+1)+' — '+E(m.qt)+'</div></div>'; });
    return h + '</div></div>';
  };
  var includeCover = r.showCover !== false;
  var contentPages = ins.snapshot.pages.filter(function(p){ return p.type!=='cover'; });
  var flaggedSectionHtml = r.showFlaggedItems ? buildFlagged() : '';
  var scoreSectionHtml = r.showScore ? buildScore() : '';
  var mediaSectionHtml = buildMedia();
  var tocEntries = [];
  var currentPageNumber = 1;

  if (includeCover) {
    tocEntries.push({ label: r.title || ins.templateName || 'Portada', page: currentPageNumber });
    currentPageNumber += 1;
  }

  if (r.showTOC) currentPageNumber += 1;

  contentPages.forEach(function(p) {
    tocEntries.push({ label: p.name, page: currentPageNumber });
    currentPageNumber += 1;
  });

  if (flaggedSectionHtml) {
    tocEntries.push({ label: 'Elementos senalados y acciones', page: currentPageNumber });
    currentPageNumber += 1;
  }

  if (scoreSectionHtml) {
    tocEntries.push({ label: 'Puntuacion', page: currentPageNumber });
    currentPageNumber += 1;
  }

  if (mediaSectionHtml) {
    tocEntries.push({ label: 'Resumen de archivos multimedia', page: currentPageNumber });
  }

  var body = '';
  if (includeCover) {
    body += '<div class="pdf-page cover-page">'+cover+'</div>';
    body += '<div class="pb"></div>';
  }
  if (r.showTOC) {
    body += '<div class="pdf-page"><div class="sh">INDICE DE CONTENIDOS</div><div style="padding:12px">';
    tocEntries.forEach(function(entry){
      body += '<div style="display:flex;align-items:baseline;padding:4px 0;font-size:10pt"><span>'+E(entry.label)+'</span><span style="flex:1;border-bottom:1px dotted #9b9b97;margin:0 6px;height:1em"></span><span style="font-size:9pt;color:#6b6b68">'+entry.page+'</span></div>';
    });
    body += '</div></div><div class="pb"></div>';
  }
  contentPages.forEach(function(p,i) {
    if(i>0) body += '<div class="pb"></div>';
    body += '<div class="pdf-page"><div class="pt">'+E(p.name)+'</div>';
    body += buildPageContent(p);
    body += '</div>';
  });
  body += flaggedSectionHtml;
  body += scoreSectionHtml;
  body += buildSig();
  body += mediaSectionHtml;
  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>'+css+'</style></head><body>';
  html += '<div class="pdf-hdr"><span class="hdr-left">' + (b.pdfLogoDataUri ? '<img class="hdr-logo" src="' + b.pdfLogoDataUri + '" alt="Logo">' : '') + '<span class="tn">'+E(ins.templateName)+'</span></span><span>'+fmtShort(ins.completedAt||ins.startedAt)+(inspector?' / '+E(inspector):'')+'</span></div>';
  html += '<div class="pdf-ftr"><span>'+E(ins.templateName.slice(0,45))+'</span><span class="br">'+E(b.footerText || b.appName || 'Auditor Libre')+'</span><span>Pagina <span class="pageNumber"></span></span></div>';
